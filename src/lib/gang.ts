...
export function getSortedTasks(ns: NS, member: string, sortBy = GainType.Money): string[] {
    ...
    if (sortBy == GainType.Money) {
        cmp = (t1: string, t2: string) => getTaskMoneyGain(ns, member, t1) - getTaskMoneyGain(ns, member, t2) ||
            ns.gang.getTaskStats(t1).baseMoney - ns.gang.getTaskStats(t2).baseMoney
    } else if (sortBy == GainType.Respect) {
        cmp = (t1: string, t2: string) => getTaskRespectGain(ns, member, t1) - getTaskRespectGain(ns, member, t2) ||
            ns.gang.getTaskStats(t1).baseRespect - ns.gang.getTaskStats(t2).baseRespect
    } else if (sortBy == GainType.WantedLevel) {
        cmp = (t1: string, t2: string) => getTaskWantedLevelGain(ns, member, t1) - getTaskWantedLevelGain(ns, member, t2) ||
            ns.gang.getTaskStats(t1).baseWanted - ns.gang.getTaskStats(t2).baseWanted
    }
